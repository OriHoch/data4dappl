...
from ckan.lib.fanstatic_resources import log, min_path


def fanstatic_resources_create_library(name, path, depend_base=True, supersedes=None):
    ...
    def create_resource(path, lib_name, count, inline=False):
        ...
        if supersedes:
            for supersede_path, superseded_resource in supersedes.items():
                if path == supersede_path:
                    superseded_library, superseded_path = superseded_resource
                    for _library in get_library_registry().values():
                        if _library.name == superseded_library:
                            kw['supersedes'] = [_library.known_resources[superseded_path]]
    ...
    module = sys.modules['ckan.lib.fanstatic_resources']
...
def toolkit_add_resource(path, name, supersedes=None):
    # Modified toolkit add_resource function to support supersedes parameter for the created resource
    import inspect
    import os
    frame, filename, line_number, function_name, lines, index = inspect.getouterframes(inspect.currentframe())[1]
    this_dir = os.path.dirname(filename)
    absolute_path = os.path.join(this_dir, path)
    fanstatic_resources_create_library(name, absolute_path, supersedes=supersedes)
